...
class Builder:
    ...
    def build(self, bundles: list[str]) -> None:
        ...
        self.write_ncx_document()
        self.write_package_document()

    def write_mimetype_file(self) -> None:
        mimetype_file = MimetypeFile()
        mimetype_file.epub2(Path(self.destination, self.project.MIMETYPE_FILE))
    ...
    def write_cover(self) -> None:
        ...
        self.resource_manager.resources[cover] = EPUBResource(cover)
    ...
    def write_navigation_document(self) -> None:
        ...
        navigation_document.epub2(
            Path(self.resource_manager.root, document_path)
        )

    def write_ncx_document(self) -> None:
        document_path = Path(self.project.NCX_DOCUMENT)
        self.resource_manager.resources[document_path] = EPUBResource(
            document_path
        )
        ncx_document = NCXDocument(
            self.project.nav_trees,
    ...
    def write_package_document(self) -> None:
        toc = Path(self.project.NCX_DOCUMENT)
        self.resource_manager.add_id_counts()
        package_document = EPUB2PackageDocument(
